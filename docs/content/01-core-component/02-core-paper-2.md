# Core Paper 2: Legislation, Business, Emerging Issues, Environments and Security
...
Organisations exist to provide a **service** or **product**, across sectors: **private** (SME, large enterprise, NGO), **public**, and **voluntary/charity** (not-for-profit). Business models include **B2C**, **B2B**, and **B2M**. Stakeholders split into **internal** (owners, directors, employees) and **external** (customers/clients, suppliers, shareholders, outsourced services, investors/funders, government).
...
**Risks**: security breaches (loss of confidentiality/integrity/availability), privacy breaches (personal or business information), regulatory/legal non-compliance, audience exclusion (bias, poor UX), emerging rival technologies, and technical issues (over-reliance, system failure, systems not fit for purpose).
...
**External triggers** (a useful PESTLE-style list): political (government change, conflict, shifting priorities), economic (new services, recession, inflation, interest rates, consumer trends, new competitors, new markets), social (demographic/social trends, remote working, cultural expectations), technological (new tech, obsolescence, system failure, zero-day vulnerabilities), legal (new/changed legislation), environmental (sustainability, pandemics, natural disasters).

Organisations **respond to change** through new/amended policies, business processes (staffing, delivery schedules, opening hours), products/services (new, next-gen, or minor updates), digital systems (back-end or customer-facing), improved training, or restructuring.
...
**On organisational culture**: changing communication methods, higher productivity/availability expectations, more staff monitoring, new working practices (remote/hybrid/in-office), automation (including AI).
...
Physical computer types: personal computers, mobile devices, servers, embedded devices. Hardware components: input/output devices; processors (cores, clock speed, cache, mobile processors); main memory (RAM, ROM); secondary storage (magnetic, solid state, optical, RAID 1/5/10, NAS, SAN); motherboard; GPUs; network interfaces (PCI, USB); cooling (air, liquid); sensors.
...
Components: virtual machines (virtual PC/switch/router clients, servers), hypervisors (Type 1, Type 2). Key features: increased security, managed execution, sharing, aggregation, emulation, isolation, portability. Benefits: cost-effective at scale, easy management, resilience, potentially lower carbon footprint, better disaster recovery, better testing environments, education/training uses. Drawbacks: extra hardware load, slower execution, risk of misleading performance figures.

### 7.5 Cloud environments

Cloud types: **private**, **public**. Benefits: portability, elasticity, fewer storage limitations, cost-effectiveness. Delivery models and who's responsible for what:

| Model | Client manages | Provider manages |
|---|---|---|
| **IaaS** | Application software, system software (middleware, OS), runtime, data, user accounts | Virtualisation, hardware (servers, network, storage) |
| **PaaS** | Application software, data, user accounts | Virtualisation, hardware, system software, runtime |
| **SaaS** | User accounts, data | Virtualisation, hardware, system software, runtime, application software |
...
Confidential information organisations hold: HR data (salaries, staff details), commercially sensitive data (client/stakeholder details, IP, sales figures, contracts), and access information (usernames, passwords, MFA details, PINs, access codes, passphrases, biometric data). Confidentiality matters because it prevents competitor poaching/undercutting, protects privacy, protects IP and client relationships, and prevents unauthorised access. Failing to protect it can cause regulatory non-compliance, loss of trust, reputational damage, financial loss (fines, refunds, lost earnings/contracts), legal action, and reduced security.
...
**Physical vulnerabilities**: lack of/poor access control (entry systems, no tailgating, complex/rotated codes, monitoring, access audits); nature of the location (shoulder surfing, environmental risk, vandalism); poor system robustness (need for rugged hardware); natural disasters.
...
- [Cyber Security Occupational Specialism](../02-occupational-specialisms/cyber-security/01-security-procedures-and-controls.md)
